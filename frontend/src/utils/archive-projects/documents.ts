...
export interface ArchiveDocumentInfo {
  id: string;
  fileName: string;
  originalName: string;
  fileType: string;
...
export interface ArchiveDocumentSummary {
  ...
  oldestDocument: ArchiveDocumentInfo | null;
  newestDocument: ArchiveDocumentInfo | null;
}

export interface ArchiveProjectDocumentsResponse {
  success: boolean;
  message: string;
  data: {
    projectInfo: {
    ...
    documentGroups: {
      projectDocuments: ArchiveDocumentGroup;
      physicalProgressDocuments: ArchiveDocumentGroup;
      financialProgressDocuments: ArchiveDocumentGroup;
      queryDocuments: ArchiveDocumentGroup;
    };
    summary: ArchiveDocumentSummary;
  };
  metadata: {
    retrievedAt: string;
    projectId: string;
 ...
 */
export async function getArchiveProjectDocuments(
  projectId: string
): Promise<ArchiveProjectDocumentsResponse> {
  try {
    ...
    // Update this endpoint based on your API structure
    const response = await fetch(
      `${API_URL}/archive-project/${projectId}/documents`,
      {
        method: "GET",
        headers: {
    ...
    const data: ArchiveProjectDocumentsResponse = await response.json();
    return data;
  } catch (error) {
...
/**
 * Download a document from archive project
 */
export async function downloadArchiveDocument(
  downloadURL: string,
  fileName: string
): Promise<void> {
  try {
    ...
    console.error("Error downloading archive document:", error);
    throw new Error("Failed to download document");
  }
}

/**
 * Preview/View a document from archive project
 */
export async function previewArchiveDocument(
  downloadURL: string,
  fileName: string,
  fileType: string
): Promise<void> {
  try {
    ...
    if (previewableTypes.includes(fileType.toLowerCase())) {
      ...
      await downloadArchiveDocument(downloadURL, fileName);
    }
  } catch (error) {
    console.error("Error previewing archive document:", error);
    throw new Error("Failed to preview document");
  }
}

/**
 * Get file type icon class for archive documents
 */
export function getArchiveFileTypeColor(fileType: string): string {
...
/**
 * Get category color for archive document category badges
 */
export function getArchiveCategoryColor(category: string): string {
  const colorMap: { [key: string]: string } = {
...
/**
 * Get subcategory color for archive document subcategory badges
 */
export function getArchiveSubcategoryColor(subcategory: string): string {
  const colorMap: { [key: string]: string } = {
 ...
 */
export function formatArchiveDateTime(dateString: string): string {
